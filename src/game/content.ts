...
export const CARDS: Record<string, CardDefinition> = {
  ...
  tape: { id: 'tape', name: 'Measure Once', cost: 1, type: 'skill', target: 'enemy', description: 'Apply 5 Exposed. The next hit deals 5 extra damage.', flavor: 'We are absolutely cutting twice.', icon: 'tape', effects: [{ kind: 'exposed', amount: 5, recipient: 'target' }] },
...
// The opening five teach defense, setup/payoff, and banking. Later draws are seeded.
export const STARTER_DECK = ['hammer', 'vest', 'tape', 'heavy', 'coffee', 'hammer', 'vest', 'brace', 'toolbox', 'hammer', 'vest', 'brace'];
...
export function enemyIntent(turn: number): { slot: number; action: EnemyAction }[] {
  ...
  return [{ slot: heavy ? 2 : 3, action: { kind: 'enemy', actor: 'guard', target: 'bob', name: heavy ? 'Excessive Force' : 'Receipt Check', description: `Deal ${heavy ? 12 : 8} damage to Bob.`, effects: [{ kind: 'damage', amount: heavy ? 12 : 8, recipient: 'target' }] } }];
}
